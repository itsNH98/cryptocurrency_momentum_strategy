...
keywords <- c('Aave', 'Algorand', 'Alpha Finance', 'Aragon', 'Balancer','Basic Attention Token', 
              'Bitcoin Cash', 'Binance Coin', 'Bitcoin SV', 'Bitcoin', 'Bitcoin Gold', 'Cardano',
              'Compound', 'Crypto.com Coin', 'Curve DAO Token', 'Civic', 'Dash', 'Decred', 'DigiByte', 'Dogecoin',
              'Polkadot', 'Dragonchain', 'aelf', 'Ethereum Classic', 'Ethereum', 'FTX Token', 'FunFair', 'Gas Coin',
              'Gnosis', 'Golem', 'Grin', 'HedgeTrade', 'Huobi Token', 'Internet Computer', 'Kyber Network', 'Aave',
              'Chainlink', 'Loom Network', 'Livepeer', 'Litecoin', 'MaidSafeCoin', 'Decentraland', 'Maker', 'NEO',
              'Nexus Mutual', 'OMG Network', 'Paxos Standard', 'Paxos Gold', 'TenX', 'Perpetual Protocol',
              'Polymath', 'Power Ledger', 'Populous', 'QASH', 'Quant', 'Ren', 'Augur', 'Sai', 'Status',
              'Synthetix Network Token', 'Serum', 'SushiSwap', 'Swerve', 'TRON', 'UMA', 'Uniswap', 'Vertcoin',
              'Wrapped NXM', 'Waltonchain', 'Tether Gold', 'NEM', 'Stellar', 'Monero', 'XRP', 'Tezos', 'Verge',
              'yearn.finance', 'Zcash', '0x')


# Dates vectors
# Returns data starts in early 2014
# We need overlapping dates to apply the knitting algorithm
weekGT <- c("2014-01-01 2016-12-31",
            "2016-01-01 2018-12-31",
            "2018-01-01 2020-12-31",
            "2020-01-01 2023-04-08") 
...
for(i in 1:length(keywords)) {
  ...
  for (j in 1:length(weekGT)) {
    alt = gtrends(keywords[i], geo = 'US', gprop="web", time = weekGT[j], onlyInterest = TRUE)
...
# for variable in the length of all variables in cryptoALL 
  ...
  res = try( 
    ...
    for (j in 1:(length(cryptoALL[[i]])-1)) {
      ...
      if(j==1){
        ...
        samA = cryptoALL[[i]][[j]][dateEmpA[[j]]]   #Echantillon A (le plus vieux)
        samB = cryptoALL[[i]][[j+1]][dateEmpB[[j]]] #Echantillon B
        
        
        # We regress the set of new values on the set of old values to find the scale
        reg = lm(samB~samA-1)                           #Modele sans constante
...
googleTable_w_can <- googleTable_w
rownames(googleTable_w_can) <- googleTable_w_can$Date 
googleTable_w_can <- subset(googleTable_w_can, select= -Date)
